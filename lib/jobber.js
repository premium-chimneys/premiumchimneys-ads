...
// ── Losing the refresh race is normal, and recoverable ────────────────────
//
// Jobber rotates refresh tokens: every refresh returns a new one and kills the
// old. Two deployments share this Jobber connection and this jobber_auth row —
// the book site and the agents app — and each coalesces its own refreshes in
// memory, which does nothing across processes.
//
// So both read refresh token R, both post it, Jobber honours the first and
// rejects the second with "The provided refresh token is not valid". Observed
// exactly once in production, two milliseconds apart: a webhook failed at
// 15:26:00.301 and the row was rewritten at 15:26:00.303. The webhook returned
// 500, and the Jobber request it was recording never got its row.
//
// The loser does not need to fail, though. The winner has, by then, written a
// perfectly good access token into the row they both read from — so the answer
// to a rejected refresh is to look again rather than to give up. If the stored
// refresh token has changed, somebody else rotated it and the token beside it is
// live.
//
// A lock would avoid the wasted call. It is not worth the complexity: this
// costs one extra HTTP request in a rare race and recovers inside the same
// function, where a lock adds a second failure mode that needs its own timeout.
  // Somebody else rotated it between our read and our post. Look again: the
  // process that won will write a usable token into the same row.
  //
  // Not once, though, and this is the whole subtlety. The winner writes AFTER
  // Jobber answers it, so losing the race and being told about it happens before
  // the good token lands. In the one occurrence on record the rejection was
  // logged at 15:26:00.301 and the row was rewritten at 15:26:00.303 — a single
  // immediate re-read would have seen the token it already had and given up two
  // milliseconds early.
  //
  // So it waits and looks again, a few times, over about a second. Far longer
  // than the gap needs to be, and far shorter than anything upstream notices.
  const ROTATION_RETRIES = 4
  const ROTATION_WAIT_MS = 300

  async function recoverFromRotation(err, previous) {
    if (!/rejected (401)|invalid_grant|not valid/i.test(err?.message || '')) return null

    for (let attempt = 0; attempt < ROTATION_RETRIES; attempt++) {
      const fresh = await readAuth()
      const rotated = fresh?.access_token && fresh.refresh_token !== previous
      const left = fresh?.expires_at ? new Date(fresh.expires_at).getTime() - Date.now() : 0
      if (rotated && left > 0) return fresh.access_token
...
async function doRefresh() {
  ...
  let json
  try {
    json = await tokenRequest(body)
  } catch (err) {
    const recovered = await recoverFromRotation(err, auth.refresh_token)
    if (recovered) {
      console.warn('[jobber] lost the refresh race; using the token the winner stored')
      return recovered
    }
    throw err
  }
...
export async function jobberGraphQLRaw(query, variables) {
  ...
  return {
    ...
    // Jobber's cost telemetry (extensions.cost.throttleStatus). Surfaced so the
    // retry layer can wait the real refill time instead of guessing. Present on
    // virtually every response, including THROTTLED ones.
