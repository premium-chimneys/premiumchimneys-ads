...
// ---------------------------------------------------------------------------
// When an invoice is paid, ALSO notify the review campaign in the agents app
...
async function fanOutEnroll(itemId) {
  try {
    ...
    // Swallow EVERYTHING. The enroll is best-effort; income_report is priority.
...
// On INVOICE_CREATE, ALSO tell the agents app to turn OFF Jobber's built-in
...
// own URL, a hard timeout, and it NEVER throws, changes income_report, or
// affects the webhook's response.
//
...
export async function processWebhookEvent({ topic, itemId }) {
  switch (topic) {
    ...
    case 'INVOICE_CREATE': {
      ...
      // Best-effort: disable Jobber's own review SMS on the new invoice.
      // Isolated + swallowed; the result returned to the webhook is
      // income_report's.
    ...
    case 'INVOICE_UPDATE': {
      // income_report FIRST and unchanged — fully awaited before any fan-out.
      const result = await handleInvoiceUpsert(itemId)
      // Best-effort enrol into the review campaign. Isolated + swallowed; the
      // result returned to the webhook (and its HTTP response) is
      // income_report's, never affected.
