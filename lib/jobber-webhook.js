...
// Review-guard fan-out (best-effort, fully isolated) — sibling of fanOutToMaya.
// On INVOICE_CREATE, ALSO tell Maya to turn OFF Jobber's built-in review-request
// SMS for the new invoice, so Maya is the only review sender. Same isolation
// contract as the enroll fan-out: its own env pair, a hard timeout, and it
// NEVER throws, changes income_report, or affects the webhook's response.
//
// Config mirrors the enroll fan-out: a dedicated URL (MAYA_REVIEW_GUARD_URL) and
// the SAME shared secret (MAYA_ENROLL_SECRET) sent as x-maya-enroll-secret.
// Until both are set, this is a no-op.

let reviewGuardEnvWarned = false

async function fanOutReviewGuard(itemId) {
  try {
    const url = process.env.MAYA_REVIEW_GUARD_URL
    const secret = process.env.MAYA_ENROLL_SECRET
    if (!url || !secret) {
      if (!reviewGuardEnvWarned) {
        console.log(
          JSON.stringify({ evt: 'review_guard.fanout_disabled', reason: 'MAYA_REVIEW_GUARD_URL/SECRET unset' })
        )
    ...
    // Forward just the invoice id — Maya's review-guard does its own lookup and
    // idempotency check (skips if already off), so no GraphQL is needed here.
    const controller = new AbortController()
    const timer = setTimeout(() => controller.abort(), MAYA_FANOUT_TIMEOUT_MS)
    try {
      const resp = await fetch(url, {
    ...
    // Swallow EVERYTHING. Best-effort; income_report is the priority.
...
export async function processWebhookEvent({ topic, itemId }) {
  switch (topic) {
    ...
    case 'INVOICE_CREATE': {
      ...
      // Best-effort: tell Maya to disable Jobber's own review SMS. Isolated +
      // swallowed; the result returned to the webhook is income_report's.
