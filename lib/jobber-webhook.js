...
// Runs a GraphQL query, retrying on throttle up to 4 times with backoff
// (2s, 4s, 8s, 16s). Returns the data on success. Throws on a non-throttle
// error, or if still throttled after the retries — the caller lets that bubble
// so the webhook responds 500 and Jobber retries later.
async function graphqlWithRetry(query, variables) {
  for (let attempt = 0; ; attempt += 1) {
    const res = await jobberGraphQLRaw(query, variables)
    if (res.ok) return res.data

    if (isThrottled(res) && attempt < 4) {
