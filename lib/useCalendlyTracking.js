...
export function openCalendly(extraParams) {
  if (typeof window === 'undefined') return

  const url = getCalendlyUrl('https://calendly.com/premiumchimneys/inspection')

  // Count the click regardless of whether the Calendly widget is ready yet
  if (typeof window.gtag === 'function') {
    window.gtag('event', 'calendly_click', {
      button_location: extraParams?.location || 'unknown',
    })
  }
  ...
  // Widget script not loaded yet — retry briefly instead of dropping the click
