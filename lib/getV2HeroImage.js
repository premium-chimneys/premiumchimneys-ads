...
// metroplex):
//   - chimney_core services  -> metroplex image, but only on the hub city's
//                               own page (see HUB_CITY)
...
// The chimney_core images are skylines of the hub city. Suburb visitors read a
// big-city skyline as "not a local company" and bounce (Naperville seeing
// Chicago), so the image only goes on the hub city's page. Every other city in
// the metroplex gets null and falls back to the V1 image. City slugs carry a
// state suffix and `dfw` is not a city, hence the explicit map rather than
// comparing slug to metroplex.
const HUB_CITY = {
  chicago: 'chicago-il',
  dfw: 'dallas-tx',
  austin: 'austin-tx',
...
// populated.
export async function getV2HeroImage(serviceSlug, citySlug, metroplex) {
  ...
  if (group === 'chimney_core') {
    if (!metroplex || HUB_CITY[metroplex] !== citySlug) return null
