...
// (4) Webhook topic discovery. We don't know the exact enum name, so probe the
//     likely candidates (a wrong __type name just returns null) AND list every
//     schema type name so we can spot the real one if all guesses miss. Pure
//     introspection — light, no data.
const WEBHOOK_TOPICS_QUERY = `
  query WebhookTopics {
    webhookEvent: __type(name: "WebhookEvent") { name kind enumValues { name } }
    webhookTopic: __type(name: "WebhookTopic") { name kind enumValues { name } }
    webhookTopicEnum: __type(name: "WebhookTopicEnum") { name kind enumValues { name } }
    webhookEventTopicEnum: __type(name: "WebhookEventTopicEnum") { name kind enumValues { name } }
    webhookEventTypeEnum: __type(name: "WebhookEventTypeEnum") { name kind enumValues { name } }
    appWebhookTopicEnum: __type(name: "AppWebhookTopicEnum") { name kind enumValues { name } }
    schemaTypes: __schema { types { name kind } }
  }
`

const WEBHOOK_ENUM_ALIASES = [
  'webhookEvent',
  'webhookTopic',
  'webhookTopicEnum',
  'webhookEventTopicEnum',
  'webhookEventTypeEnum',
  'appWebhookTopicEnum',
]

// Runs the discovery query and returns a compact result:
//   enums                  { <TypeName>: [topic, ...] } for any candidate that resolved
//   webhookishTypeNames    every schema type whose name contains "hook" (kind included)
...
async function fetchWebhookTopics() {
  ...
  const enums = {}
  for (const alias of WEBHOOK_ENUM_ALIASES) {
    const t = data[alias]
    if (t && Array.isArray(t.enumValues)) {
      enums[t.name] = t.enumValues.map((v) => v.name)
    }
  }

  const webhookishTypeNames = (data.schemaTypes?.types || [])
    .filter((t) => /hook/i.test(t.name))
    .map((t) => ({ name: t.name, kind: t.kind }))

  return { ok: res.ok, status: res.status, enums, webhookishTypeNames }
}
...
export async function previewInvoices() {
  ...
  return { enumValues, invoices, jobLink, webhookTopics }
}
