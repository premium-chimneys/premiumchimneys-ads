...
// (7) Assigned team member discovery. The "Team" assignment (e.g. "Leo") lives
//     separately from salesperson — likely an assignedUsers (User connection)
//     on the Assessment and/or Job. Introspect both type's fields in one call.
const TEAM_FIELDS_QUERY = `
  query TeamFields {
    assessment: __type(name: "Assessment") {
      name
...
async function fetchTeamFields() {
  const res = await callWithRetry(TEAM_FIELDS_QUERY)
  if (!res.ok) return res

  const data = res.data || {}
  const pickFields = (t) =>
    Array.isArray(t?.fields) ? t.fields.map((f) => ({ name: f.name, type: f.type })) : null

  return {
    ok: res.ok,
    status: res.status,
    assessment: { name: data.assessment?.name ?? null, fields: pickFields(data.assessment) },
    job: { name: data.job?.name ?? null, fields: pickFields(data.job) },
  }
}
...
export async function previewInvoices() {
  ...
  await sleep(1500)
  const teamFields = await fetchTeamFields()

  return { enumValues, invoices, jobLink, webhookTopics, requestFields, userName, teamFields }
}
