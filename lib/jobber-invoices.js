...
// Call once; on throttle, back off and retry with exponential waits.
async function callWithRetry(query, variables = {}, { retries = 4, baseDelayMs = 1500 } = {}) {
  let attempt = 0
  for (;;) {
    const res = await jobberGraphQLRaw(query, variables)
    if (!isThrottled(res) || attempt >= retries) return res
    await sleep(baseDelayMs * 2 ** attempt) // 1.5s, 3s, 6s, 12s
    attempt += 1
  }
}
...
const PAGE_SIZE = 100 // Jobber connections cap at 100 per page.
const INTER_PAGE_DELAY_MS = 500 // gentle spacing on top of throttle backoff.
...
function mapInvoiceToRow(inv) {
  ...
  const reportDate = inv?.issuedDate ? String(inv.issuedDate).slice(0, 10) : null
